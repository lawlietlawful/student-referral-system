...
class SeminarController extends Controller
{
    public function index(Request $request)
    {
        ...
        $calendarEvents = Seminar::all()->map(function ($seminar) {
            return [
                ...
                'url' => route('counselor.seminars.show', $seminar->id),
                'color' => $seminar->status === 'completed' ? '#10B981' : ($seminar->status === 'upcoming' ? '#3B82F6' : '#F59E0B'),
            ];
        });

        return view('counselor.seminars.index', compact(
            'seminars', 
            'totalSeminars', 
            'upcomingSessions', 
    ...
    public function show(Request $request, Seminar $seminar)
    {
        ...
        return view('counselor.seminars.show', compact('seminar', 'availableStudents', 'participants'));
    }
    ...
    {
        $seminar->load('students');
        return view('counselor.seminars.print_roster', compact('seminar'));
    }
