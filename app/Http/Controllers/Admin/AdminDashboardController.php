...
class AdminDashboardController extends Controller
{
    public function index()
    {
        ...
        $aiInsightsRaw = [
            'Excessive Absences' => 0,
            'Frequent Tardiness' => 0,
            'Behavioral Misconduct' => 0,
            'Academic Failure' => 0,
        ];

        foreach ($highRiskAssessments as $assessment) {
            if ($assessment->total_absences >= 5) $aiInsightsRaw['Excessive Absences']++;
            if ($assessment->tardiness >= 5) $aiInsightsRaw['Frequent Tardiness']++;
            if ($assessment->misconduct >= 1) $aiInsightsRaw['Behavioral Misconduct']++;
            if ($assessment->failed_subjects >= 1) $aiInsightsRaw['Academic Failure']++;
        }
